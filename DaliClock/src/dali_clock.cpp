...
void DaliClock::fill_gradient(uint32_t top_color, uint32_t bottom_color) {
...
void DaliClock::draw_digit_bitmaps(uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
  constexpr uint16_t d_linestride = digit_bits_linestride;
  constexpr uint16_t d_width      = digit_bits_linestride * 8;
  constexpr uint16_t d_height     = digit_bits_height;
  
  constexpr uint16_t s_linestride = separator_bits_linestride;
  constexpr uint16_t s_width      = separator_bits_linestride * 8;
  constexpr uint16_t s_height     = separator_bits_height;
  
  constexpr uint8_t dropShadow  = 6 * 16;
  
  const uint16_t digit_offset = w/7 * 16;
  x *= 16;
  y *= 16;
  
  constexpr uint16_t scaled_height = 200;
  
  constexpr float scale = scaled_height/digit_bits_height;
  ...
  cmd.tag(9)
     ...
     .cmd(BEGIN(RECTS))
     .cmd(VERTEX2F(0, y + scaled_height/2*16))
     .cmd(VERTEX2F(Hsize*16, Vsize*16))
     .cmd(END())
     ...
     .cmd(COLOR_RGB(0x0000FF))
     .cmd(BEGIN(BITMAPS))
     
     // Draw the separators     
     .cmd(BITMAP_LAYOUT(L1, s_linestride, s_height))
     .cmd(BITMAP_SIZE(NEAREST, BORDER, BORDER, scale * s_width, scale * s_height))
     .cmd(TAG_MASK(true))
     .tag(7).cmd(BITMAP_SOURCE(separator_bits_addr )).cmd(VERTEX2F(x + (digit_offset*2.0) + dropShadow, y + dropShadow))
     .tag(8).cmd(BITMAP_SOURCE(separator_bits_addr )).cmd(VERTEX2F(x + (digit_offset*4.5) + dropShadow, y + dropShadow))
     
     // Draw the digits
     .cmd(BITMAP_LAYOUT(L1, d_linestride, d_height))
     .cmd(BITMAP_SIZE(NEAREST, BORDER, BORDER, scale * d_width, scale * d_height))
     .tag(1).cmd(BITMAP_SOURCE(digit1_bits_addr)).cmd(VERTEX2F(x + (digit_offset*0.0) + dropShadow, y + dropShadow))
     .tag(2).cmd(BITMAP_SOURCE(digit2_bits_addr)).cmd(VERTEX2F(x + (digit_offset*1.0) + dropShadow, y + dropShadow))
     .tag(3).cmd(BITMAP_SOURCE(digit3_bits_addr)).cmd(VERTEX2F(x + (digit_offset*2.5) + dropShadow, y + dropShadow))
     .tag(4).cmd(BITMAP_SOURCE(digit4_bits_addr)).cmd(VERTEX2F(x + (digit_offset*3.5) + dropShadow, y + dropShadow))
     .tag(5).cmd(BITMAP_SOURCE(digit5_bits_addr)).cmd(VERTEX2F(x + (digit_offset*5.0) + dropShadow, y + dropShadow))
     .tag(6).cmd(BITMAP_SOURCE(digit6_bits_addr)).cmd(VERTEX2F(x + (digit_offset*6.0) + dropShadow, y + dropShadow))
     .cmd(TAG_MASK(false))
     ...
     .cmd(BITMAP_SIZE(NEAREST, BORDER, BORDER, scale * s_width, scale * s_height))
     .cmd(TAG_MASK(true))
     .tag(7).cmd(BITMAP_SOURCE(separator_bits_addr )).cmd(VERTEX2F(x + (digit_offset*2.0), y))
     .tag(8).cmd(BITMAP_SOURCE(separator_bits_addr )).cmd(VERTEX2F(x + (digit_offset*4.5), y))
     
     // Draw the digits
     .cmd(BITMAP_LAYOUT(L1, d_linestride, d_height))
     .cmd(BITMAP_SIZE(NEAREST, BORDER, BORDER, scale * d_width, scale * d_height))
     .tag(1).cmd(BITMAP_SOURCE(digit1_bits_addr)).cmd(VERTEX2F(x + (digit_offset*0.0), y))
     .tag(2).cmd(BITMAP_SOURCE(digit2_bits_addr)).cmd(VERTEX2F(x + (digit_offset*1.0), y))
     .tag(3).cmd(BITMAP_SOURCE(digit3_bits_addr)).cmd(VERTEX2F(x + (digit_offset*2.5), y))
     .tag(4).cmd(BITMAP_SOURCE(digit4_bits_addr)).cmd(VERTEX2F(x + (digit_offset*3.5), y))
     .tag(5).cmd(BITMAP_SOURCE(digit5_bits_addr)).cmd(VERTEX2F(x + (digit_offset*5.0), y))
     .tag(6).cmd(BITMAP_SOURCE(digit6_bits_addr)).cmd(VERTEX2F(x + (digit_offset*6.0), y))
     .cmd(TAG_MASK(false))
     .cmd(END())
     
      // Overlay the gradient on the clock digits
     .cmd(BITMAP_TRANSFORM_A(256 * 1))
     .cmd(BITMAP_TRANSFORM_E(256.0f / (float(scaled_height)/clock_gradient_height)))
     ...
     .cmd(BITMAP_SIZE(BILINEAR, REPEAT, BORDER, Hsize, scaled_height))
     .cmd(BEGIN(BITMAPS))
     .cmd(VERTEX2F(0, y))
     
     .cmd(BITMAP_TRANSFORM_A(256 * 1))
     .cmd(BITMAP_TRANSFORM_E(256 * 1));
}
